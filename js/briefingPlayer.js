...
$(function () {
    ...
    $mediaPlayer = $('#mediaPlayer').mediaplayer({
        ...
        onPlaynEnded: function (complete) {
            /// <summary>
            /// 學員影片播放完畢處理
            /// </summary>
            /// <param name="complete">是否正確播放完畢</param>              
            if (complete) {
                if (videoData.Playlist_Mark) {
                    FinishWatchVideo();
                }
            }
            else {
    ...
    var toggleSlideBar = $('[data-toggle="slidebar"]');
    var back = $('#back');
    var body = $('html, body');    
    ...
    toggleSlideBar.click(function () {
        var isOpen = slidebar.is(':visible');
        var icon = slidebar.find('.ui-icon');

        if (isOpen) {
            slidebar['slideUp'](250, function () {
                slidebar.addClass('hidden-xs');
            }).trigger('hide');

            //back.hide();
        }
        else {
            slidebar.removeClass('hidden-xs').hide();
            slidebar['slideDown'](250).trigger('show');
            //back.show();
        }

        toggleSlideBar.toggleClass('active');
    });

    back.click(function () {
        if (readingPostition != 0) {
            // 如果剛剛有點擊章節，將閱讀的章節顯示到螢幕中間
            body.animate({ scrollTop: readingPostition - ($(window).height() / 2) }, 500, 'swing');
        }
        else
    ...
    // 點擊影片
    $('.video-group').on('click', 'a.video', function (e) {
        e.preventDefault();
        ...
        if (_this.hasClass('new')) _this.removeClass('new');

        var videoId = _this.data("videoId");

        var videoTopic = _this.find('.video-topic').text();
        $mediaPlayer.setSource({
            videoId: videoId,
            sourceType: 'briefing',
            onComplete: OnGetMediaSourceComplete
        ...
        $('.navbar-brand.video-topic').text(videoTopic);
    ...
    //    e.preventDefault();
    //    var _this = $(this);
    //    var firstQueryNos = _this.data('firstQueryNos');
    //    var beginIndex = _this.data('beginIndex');
    //    var queryMode = _this.data('role');
    //    var countNos = 5;

    //    // 看完筆數每次查十筆
    //    if (queryMode == 'watched')
    //        countNos = 10;
    ...
    //    StartAjax("GetMoreBriefingVideo", { Begin_Index: beginIndex, Count_Nos: countNos, First_Query_Nos: firstQueryNos, Query_Mode: queryMode }, function (result) {
    //        var myPlayListTable = result;

    //        var myHtml = "";
    //        for (index in myPlayListTable) {
    //            var videoRow = myPlayListTable[index];
    //            var videoClass = "";

    //            if (videoRow.Watch_Status == "") videoClass = "new";

    //            myHtml += '<a href="#" class="list-group-item video ' + videoClass + '" data-video-id="' + videoRow.Video_Id + '">';
    //            myHtml += '<span class="colaUser" data-user-id="' + videoRow.Publish_User + '">' + videoRow.Publish_Name + '</span>：';
    //            myHtml += '<span class="video-topic">' + videoRow.Doc_Topic + '</span>';
    //            myHtml += '</a>';
    //        }

    //        $(myHtml).hide().insertBefore(_this).slideDown(200);
    //        beginIndex += countNos + 1;
    //        _this.data('beginIndex', beginIndex);

    //        // 資料查詢的筆數（Row_Index）大於第一次查詢的比數，表示後面沒有資料了，隱藏按鈕
    //        if ((beginIndex + countNos - 1) > firstQueryNos) {
    //            _this.hide();
    //        }
    ...
    $more.click(function (e) {
        ...
        var onlineTime = new Date($more.data('baseDate'));        

        // 避免重複按兩次
        $more.addClass('active');
        $more.prop('disabled', true);

        StartAjax("GetMoreWatchedVideo", { Online_Time: onlineTime }, function (result) {
            var table = result.myPlaylist;
            
            var html = '';
            for (index in table) {
                var videoRow = table[index];
                var videoClass = "";

                if (videoRow.Watch_Status == "") videoClass = "new";

                html += '<a href="#" class="list-group-item video ' + videoClass + '" data-video-id="' + videoRow.Video_Id + '">';
                html += '<span class="colaUser" data-user-id="' + videoRow.Publish_User + '">' + videoRow.Publish_Name + '</span>：';
                html += '<span class="video-topic">' + videoRow.Doc_Topic + '</span>';
                html += '</a>';
            }

            $(html).hide().insertBefore($more).slideDown(200);

            onlineTime = new Date(result.baseDate);
            $more.data('baseDate', onlineTime);
            
            
            if (table.length == 0 || onlineTime.getFullYear() === 1900) {
                $more.hide();
            }
        ...
        StartAjax("FinishWatchVideo", { Video_Id: videoId }, function (result) {            
            ...
            else
            {
                ...
                if (videoData.Finish_Mark == false) {
                    ...
                    if ($(".list-group-content.watched").length !== 0) {
    ...
    // 影片列表收放
    $('.list-group-heading').click(function () {
        var self = $(this);
        var panel = self.next();
        ...
        if (isOpen) {
            panel['slideUp'](250, function () { self.parent().toggleClass('ui-collapsible-collapsed'); }).trigger('hide');
        }
        else {
            panel['slideDown'](250).trigger('show');
            self.parent().toggleClass('ui-collapsible-collapsed');
        }
    });
        ...
        $old.stop().fadeOut(200, function () {
            ...
            $panel.stop().hide().addClass('active').fadeIn(200, function () {
                if ($panel.data('scroll') !== undefined) {
                    $panel.stop().find('.panel-group').animate({ scrollTop: $panel.data('scroll') }, 200);
                }                
